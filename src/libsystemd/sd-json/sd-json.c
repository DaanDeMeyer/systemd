...
struct sd_json_variant {
        union {
                 ...
                 * SD_JSON_VARIANT_ARRAY/SD_JSON_VARIANT_OBJECT object. (If 'embedded' is true, see below.) */
        ...
        union {
                ...
                /* Strings are placed immediately after the structure. Note that when this is a sd_json_variant embedded
                 ...
                 *
                 * Note that because we want to allocate arrays of the sd_json_variant structure we specify [0] here,
 ...
 *    3. A 'magic' one, i.e. one of the special SD_JSON_VARIANT_MAGIC_XYZ values, that encode a few very basic values directly in the pointer.
 ...
 *    _SD_JSON_VARIANT_MAGIC_MAX (which is pretty low, within the first memory page, which is special on Linux and other
 ...
 *    access. The SD_JSON_VARIANT_STRING_CONST() macro encodes strings as sd_json_variant* pointers, with the bit set. */

static bool sd_json_variant_is_magic(const sd_json_variant *v) {
        if (!v)
                return false;

        return v < _SD_JSON_VARIANT_MAGIC_MAX;
}

static bool sd_json_variant_is_const_string(const sd_json_variant *v) {

        if (v < _SD_JSON_VARIANT_MAGIC_MAX)
                return false;
...
static bool sd_json_variant_is_regular(const sd_json_variant *v) {
...
static sd_json_variant *sd_json_variant_dereference(sd_json_variant *v) {
        ...
        if (!sd_json_variant_is_regular(v))
                return v;

        if (!v->is_reference)
                return v;

        return sd_json_variant_dereference(v->reference);
}

static uint16_t sd_json_variant_depth(sd_json_variant *v) {

        v = sd_json_variant_dereference(v);
        if (!v)
                return 0;
...
static sd_json_variant *sd_json_variant_formalize(sd_json_variant *v) {
        ...
        case SD_JSON_VARIANT_BOOLEAN:
                return sd_json_variant_boolean(v) ? SD_JSON_VARIANT_MAGIC_TRUE : SD_JSON_VARIANT_MAGIC_FALSE;

        case SD_JSON_VARIANT_NULL:
                return SD_JSON_VARIANT_MAGIC_NULL;

        case SD_JSON_VARIANT_INTEGER:
                return sd_json_variant_integer(v) == 0 ? SD_JSON_VARIANT_MAGIC_ZERO_INTEGER : v;

        case SD_JSON_VARIANT_UNSIGNED:
                return sd_json_variant_unsigned(v) == 0 ? SD_JSON_VARIANT_MAGIC_ZERO_UNSIGNED : v;

        case SD_JSON_VARIANT_REAL:
                DISABLE_WARNING_FLOAT_EQUAL;
                return sd_json_variant_real(v) == 0.0 ? SD_JSON_VARIANT_MAGIC_ZERO_REAL : v;
                REENABLE_WARNING;

        case SD_JSON_VARIANT_STRING:
                return isempty(sd_json_variant_string(v)) ? SD_JSON_VARIANT_MAGIC_EMPTY_STRING : v;

        case SD_JSON_VARIANT_ARRAY:
                return sd_json_variant_elements(v) == 0 ? SD_JSON_VARIANT_MAGIC_EMPTY_ARRAY : v;

        case SD_JSON_VARIANT_OBJECT:
                return sd_json_variant_elements(v) == 0 ? SD_JSON_VARIANT_MAGIC_EMPTY_OBJECT : v;
...
static sd_json_variant *sd_json_variant_conservative_formalize(sd_json_variant *v) {

        /* Much like sd_json_variant_formalize(), but won't simplify if the variant has a source/line location attached to
        ...
        return sd_json_variant_formalize(v);
}

static int sd_json_variant_new(sd_json_variant **ret, sd_json_variant_type_t type, size_t space) {
        sd_json_variant *v;
...
int sd_json_variant_new_integer(sd_json_variant **ret, int64_t i) {
        ...
        if (i == 0) {
                *ret = SD_JSON_VARIANT_MAGIC_ZERO_INTEGER;
                return 0;
        }

        r = sd_json_variant_new(&v, SD_JSON_VARIANT_INTEGER, sizeof(i));
        if (r < 0)
                return r;
...
int sd_json_variant_new_unsigned(sd_json_variant **ret, uint64_t u) {
        ...
        assert_return(ret, -EINVAL);
        if (u == 0) {
                *ret = SD_JSON_VARIANT_MAGIC_ZERO_UNSIGNED;
                return 0;
        }

        r = sd_json_variant_new(&v, SD_JSON_VARIANT_UNSIGNED, sizeof(u));
        if (r < 0)
                return r;
...
int sd_json_variant_new_real(sd_json_variant **ret, double d) {
        ...
        DISABLE_WARNING_FLOAT_EQUAL;
        if (d == 0.0) {
                *ret = SD_JSON_VARIANT_MAGIC_ZERO_REAL;
                return 0;
        }
        REENABLE_WARNING;
        ...
        if (isnan(d) || isinf(d)) {
                *ret = SD_JSON_VARIANT_MAGIC_NULL;
                return 0;
        }

        r = sd_json_variant_new(&v, SD_JSON_VARIANT_REAL, sizeof(d));
        if (r < 0)
                return r;
...
int sd_json_variant_new_boolean(sd_json_variant **ret, bool b) {
        ...
        if (b)
                *ret = SD_JSON_VARIANT_MAGIC_TRUE;
        else
                *ret = SD_JSON_VARIANT_MAGIC_FALSE;
...
int sd_json_variant_new_stringn(sd_json_variant **ret, const char *s, size_t n) {
        ...
        if (n == SIZE_MAX) /* determine length automatically */
                ...
                return -EINVAL;
        if (n == 0) {
                *ret = SD_JSON_VARIANT_MAGIC_EMPTY_STRING;
                return 0;
        }
        ...
        r = sd_json_variant_new(&v, SD_JSON_VARIANT_STRING, n + 1);
        if (r < 0)
                return r;
...
static void sd_json_variant_set(sd_json_variant *a, sd_json_variant *b) {
        assert(a);

        b = sd_json_variant_dereference(b);
        if (!b) {
        ...
        case SD_JSON_VARIANT_OBJECT:
                a->is_reference = true;
                a->reference = sd_json_variant_ref(sd_json_variant_conservative_formalize(b));
                break;
...
static void sd_json_variant_copy_source(sd_json_variant *v, sd_json_variant *from) {
        assert(v);

        if (!sd_json_variant_is_regular(from))
                return;
...
int sd_json_variant_new_array(sd_json_variant **ret, sd_json_variant **array, size_t n) {
        ...
        assert_return(ret, -EINVAL);
        if (n == 0) {
                *ret = SD_JSON_VARIANT_MAGIC_EMPTY_ARRAY;
                return 0;
        }
        ...
        *v = (sd_json_variant) {
        ...
        for (v->n_elements = 0; v->n_elements < n; v->n_elements++) {
                ...
                *w = (sd_json_variant) {
                ...
                sd_json_variant_set(w, c);
                sd_json_variant_copy_source(w, c);

                if (!sd_json_variant_is_normalized(c))
                        normalized = false;
        }
...
int sd_json_variant_new_object(sd_json_variant **ret, sd_json_variant **array, size_t n) {
        ...
        assert_return(ret, -EINVAL);
        if (n == 0) {
                *ret = SD_JSON_VARIANT_MAGIC_EMPTY_OBJECT;
                return 0;
        }
        assert_return(array, -EINVAL);
...
static size_t sd_json_variant_size(sd_json_variant* v) {
...
static void sd_json_variant_free_inner(sd_json_variant *v, bool force_sensitive) {
        bool sensitive;
        ...
        if (IN_SET(v->type, SD_JSON_VARIANT_ARRAY, SD_JSON_VARIANT_OBJECT)) {
                ...
                for (i = 0; i < v->n_elements; i++)
                        sd_json_variant_free_inner(v + 1 + i, sensitive);
        }

        if (sensitive)
                explicit_bzero_safe(v, sd_json_variant_size(v));
}
...
sd_json_variant *sd_json_variant_unref(sd_json_variant *v) {
        ...
        if (v->is_embedded)
                ...
                if (v->n_ref == 0) {
                        sd_json_variant_free_inner(v, false);
                        free(v);
                }
        }
...
const char *sd_json_variant_string(sd_json_variant *v) {
        if (!v)
                return NULL;
        if (v == SD_JSON_VARIANT_MAGIC_EMPTY_STRING)
                return "";
        if (sd_json_variant_is_magic(v))
                goto mismatch;
        if (sd_json_variant_is_const_string(v)) {
...
bool sd_json_variant_boolean(sd_json_variant *v) {
        if (!v)
                goto mismatch;
        if (v == SD_JSON_VARIANT_MAGIC_TRUE)
                return true;
        if (v == SD_JSON_VARIANT_MAGIC_FALSE)
                return false;
        if (!sd_json_variant_is_regular(v))
                goto mismatch;
...
int64_t sd_json_variant_integer(sd_json_variant *v) {
        if (!v)
                goto mismatch;
        if (v == SD_JSON_VARIANT_MAGIC_ZERO_INTEGER ||
            v == SD_JSON_VARIANT_MAGIC_ZERO_UNSIGNED ||
            v == SD_JSON_VARIANT_MAGIC_ZERO_REAL)
                return 0;
        if (!sd_json_variant_is_regular(v))
                goto mismatch;
        if (v->is_reference)
...
sd_json_variant_type_t sd_json_variant_type(sd_json_variant *v) {
        ...
        if (v == SD_JSON_VARIANT_MAGIC_TRUE || v == SD_JSON_VARIANT_MAGIC_FALSE)
                return SD_JSON_VARIANT_BOOLEAN;

        if (v == SD_JSON_VARIANT_MAGIC_NULL)
                return SD_JSON_VARIANT_NULL;

        if (v == SD_JSON_VARIANT_MAGIC_ZERO_INTEGER)
                return SD_JSON_VARIANT_INTEGER;

        if (v == SD_JSON_VARIANT_MAGIC_ZERO_UNSIGNED)
                return SD_JSON_VARIANT_UNSIGNED;

        if (v == SD_JSON_VARIANT_MAGIC_ZERO_REAL)
                return SD_JSON_VARIANT_REAL;

        if (v == SD_JSON_VARIANT_MAGIC_EMPTY_STRING)
                return SD_JSON_VARIANT_STRING;

        if (v == SD_JSON_VARIANT_MAGIC_EMPTY_ARRAY)
                return SD_JSON_VARIANT_ARRAY;

        if (v == SD_JSON_VARIANT_MAGIC_EMPTY_OBJECT)
...
bool sd_json_variant_has_type(sd_json_variant *v, sd_json_variant_type_t type) {
        ...
        if ((v == SD_JSON_VARIANT_MAGIC_ZERO_INTEGER || v == SD_JSON_VARIANT_MAGIC_ZERO_UNSIGNED || v == SD_JSON_VARIANT_MAGIC_ZERO_REAL) &&
...
size_t sd_json_variant_elements(sd_json_variant *v) {
        if (!v)
                return 0;
        if (v == SD_JSON_VARIANT_MAGIC_EMPTY_ARRAY ||
            v == SD_JSON_VARIANT_MAGIC_EMPTY_OBJECT)
                return 0;
        if (!sd_json_variant_is_regular(v))
                goto mismatch;
        if (!IN_SET(v->type, SD_JSON_VARIANT_ARRAY, SD_JSON_VARIANT_OBJECT))
...
sd_json_variant *sd_json_variant_by_index(sd_json_variant *v, size_t idx) {
        ...
        return sd_json_variant_conservative_formalize(v + 1 + idx);
...
sd_json_variant *sd_json_variant_by_key_full(sd_json_variant *v, const char *key, sd_json_variant **ret_key) {
        ...
        if (v->sorted) {
                ...
                while (b > a) {
                        ...
                        i = (a + b) / 2;
                        p = sd_json_variant_dereference(v + 1 + i*2);
                        ...
                        if (c == 0) {
                                if (ret_key)
                                        *ret_key = sd_json_variant_conservative_formalize(v + 1 + i*2);

                                return sd_json_variant_conservative_formalize(v + 1 + i*2 + 1);
                        } else if (c < 0)
                                b = i;
                        else
                                a = i + 1;
        ...
        for (i = 0; i < v->n_elements; i += 2) {
                ...
                p = sd_json_variant_dereference(v + 1 + i);
                ...
                if (streq(sd_json_variant_string(p), key)) {
                        ...
                        if (ret_key)
                                *ret_key = sd_json_variant_conservative_formalize(v + 1 + i);

                        return sd_json_variant_conservative_formalize(v + 1 + i + 1);
                }
        }
...
bool sd_json_variant_equal(sd_json_variant *a, sd_json_variant *b) {
        ...
        a = sd_json_variant_formalize(a);
        b = sd_json_variant_formalize(b);
        ...
        case SD_JSON_VARIANT_REAL:
                DISABLE_WARNING_FLOAT_EQUAL;
                return sd_json_variant_real(a) == sd_json_variant_real(b);
                REENABLE_WARNING;
...
static void sd_json_variant_propagate_sensitive(sd_json_variant *from, sd_json_variant *to) {
...
int sd_json_variant_get_source(sd_json_variant *v, const char **ret_source, unsigned *ret_line, unsigned *ret_column) {
        ...
        if (ret_source)
                *ret_source = sd_json_variant_is_regular(v) && v->source ? v->source->name : NULL;

        if (ret_line)
                *ret_line = sd_json_variant_is_regular(v) ? v->line : 0;

        if (ret_column)
                *ret_column = sd_json_variant_is_regular(v) ? v->column : 0;
...
int sd_json_variant_filter(sd_json_variant **v, char **to_remove) {
        ...
        for (i = 0; i < sd_json_variant_elements(*v); i += 2) {
                sd_json_variant *p;
        ...
        sd_json_variant_propagate_sensitive(*v, w);
...
int sd_json_variant_set_field_integer(sd_json_variant **v, const char *field, int64_t i) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *m = NULL;
        int r;
...
int sd_json_variant_set_field_unsigned(sd_json_variant **v, const char *field, uint64_t u) {
        _cleanup_(sd_json_variant_unrefp) sd_json_variant *m = NULL;
        int r;
...
int sd_json_variant_merge(sd_json_variant **v, sd_json_variant *m) {
        ...
        m = sd_json_variant_dereference(m);
        ...
        sd_json_variant_propagate_sensitive(*v, w);
        sd_json_variant_propagate_sensitive(m, w);
...
int sd_json_variant_append_array(sd_json_variant **v, sd_json_variant *element) {
        ...
        if (blank)
                ...
                array = new(sd_json_variant*, sd_json_variant_elements(*v) + 1);
                if (!array)
                        return -ENOMEM;

                for (i = 0; i < sd_json_variant_elements(*v); i++)
                        array[i] = sd_json_variant_by_index(*v, i);

                array[i] = element;

                r = sd_json_variant_new_array(&nv, array, i + 1);
        }
        if (r < 0)
                return r;

        sd_json_variant_propagate_sensitive(*v, nv);
...
static int sd_json_variant_copy(sd_json_variant **nv, sd_json_variant *v) {
        sd_json_variant_type_t t;
        sd_json_variant *c;
        JsonValue value;
        const void *source;
        ...
        default:
                ...
                c->type = t;
                c->is_reference = true;
                c->reference = sd_json_variant_ref(sd_json_variant_formalize(v));
        ...
        sd_json_variant_propagate_sensitive(v, c);
...
static int sd_json_variant_set_source(sd_json_variant **v, JsonSource *source, unsigned line, unsigned column) {
        sd_json_variant *w;
        int r;
        ...
        r = sd_json_variant_copy(&w, *v);
        if (r < 0)
                return r;

        assert(sd_json_variant_is_regular(w));
...
static int sd_json_parse_string(const char **p, char **ret) {
        _cleanup_free_ char *s = NULL;
        size_t n = 0;
        const char *c;
...
static int sd_json_parse_number(const char **p, JsonValue *ret) {
        ...
        case STATE_VALUE:
                ...
                if (*c == '{') {
                        ...
                        *state = INT_TO_PTR(STATE_VALUE_POST);
                        ...
                        r = sd_json_parse_string(&c, ret_string);
                        if (r < 0)
                                return r;
                        ...
                        *state = INT_TO_PTR(STATE_VALUE_POST);
                        ...
                        r = sd_json_parse_number(&c, ret_value);
                        if (r < 0)
                                return r;
...
static void json_stack_release(JsonStack *s) {
        ...
        sd_json_variant_unref_many(s->elements, s->n_elements);
        s->elements = mfree(s->elements);
}

static int sd_json_parse_internal(
                const char **input,
                JsonSource *source,
        ...
        for (;;) {
                ...
                if (add) {
                        ...
                        (void) sd_json_variant_set_source(&add, source, line_token, column_token);
...
int sd_json_parse(const char *input, sd_json_parse_flags_t flags, sd_json_variant **ret, unsigned *ret_line, unsigned *ret_column) {
        return sd_json_parse_internal(&input, NULL, flags, ret, ret_line, ret_column, false);
}
...
int sd_json_parse_continue(const char **p, sd_json_parse_flags_t flags, sd_json_variant **ret, unsigned *ret_line, unsigned *ret_column) {
        return sd_json_parse_internal(p, NULL, flags, ret, ret_line, ret_column, true);
}
...
int sd_json_parse_file_at(FILE *f, int dir_fd, const char *path, sd_json_parse_flags_t flags, sd_json_variant **ret, unsigned *ret_line, unsigned *ret_column) {
        ...
        p = text;
        return sd_json_parse_internal(&p, source, flags, ret, ret_line, ret_column, false);
}
...
int sd_json_parse_file(FILE *f, const char *path, sd_json_parse_flags_t flags, sd_json_variant **ret, unsigned *ret_line, unsigned *ret_column) {
        return sd_json_parse_file_at(f, AT_FDCWD, path, flags, ret, ret_line, ret_column);
}
...
int sd_json_buildv(sd_json_variant **ret, va_list ap) {
        ...
        for (;;) {
                ...
                case _SD_JSON_BUILD_VARIANT:
                        ...
                        if (!add)
                                add = SD_JSON_VARIANT_MAGIC_NULL;
                        else
                ...
                case _SD_JSON_BUILD_BASE64: {
                        ...
                        if (current->n_suppress == 0) {
                                r = sd_json_variant_new_base64(&add, p, n);
                                if (r < 0)
                                        goto finish;
                        }
                ...
                case _SD_JSON_BUILD_HEX: {
                        ...
                        if (current->n_suppress == 0) {
                                r = sd_json_variant_new_hex(&add, p, n);
                                if (r < 0)
                                        goto finish;
                        }
                ...
                case _SD_JSON_BUILD_ID128: {
                        ...
                        if (current->n_suppress == 0) {
                                r = sd_json_variant_new_id128(&add, *id);
                                if (r < 0)
                                        goto finish;
                        }
                ...
                case _SD_JSON_BUILD_PAIR_UNSIGNED_NON_ZERO: {
                        const char *n;
                        uint64_t u;
                ...
                case _SD_JSON_BUILD_PAIR_FINITE_USEC: {
                        const char *n;
                        usec_t u;
                ...
                case _SD_JSON_BUILD_PAIR_STRING_NON_EMPTY: {
                        const char *n, *s;
                ...
                case _SD_JSON_BUILD_PAIR_STRV_NON_EMPTY: {
                        const char *n;
                        char **l;
                ...
                case _SD_JSON_BUILD_PAIR_VARIANT_NON_NULL: {
                        sd_json_variant *v;
                        const char *n;
                ...
                case _SD_JSON_BUILD_PAIR_IN4_ADDR_NON_NULL: {
                ...
                case _SD_JSON_BUILD_PAIR_IN6_ADDR_NON_NULL: {
                ...
                case _SD_JSON_BUILD_PAIR_IN_ADDR_NON_NULL: {
                ...
                case _SD_JSON_BUILD_PAIR_ETHER_ADDR_NON_NULL: {
                ...
                case _SD_JSON_BUILD_PAIR_HW_ADDR_NON_NULL: {
...
int sd_json_dispatch(sd_json_variant *v, const sd_json_dispatch_t table[], sd_json_dispatch_callback_t bad, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        for (i = 0; i < n; i += 2) {
                ...
                if (p->name) { /* Found a matching entry! :-) */
                        ...
                        if (p->callback) {
                                r = p->callback(sd_json_variant_string(key), value, merged_flags, (uint8_t*) userdata + p->offset);
                                if (r < 0) {
                                        if (merged_flags & SD_JSON_PERMISSIVE)
                                                continue;
...
int sd_json_dispatch_int32(const char *name, sd_json_variant *variant, sd_json_dispatch_flags_t flags, void *userdata) {
        ...
        if (sd_json_variant_integer(variant) < INT32_MIN || sd_json_variant_integer(variant) > INT32_MAX)
                return json_log(variant, flags, SYNTHETIC_ERRNO(ERANGE), "JSON field '%s' out of bounds.", strna(name));

        *i = (int32_t) sd_json_variant_integer(variant);
        return 0;
}
...
int sd_json_variant_sort(sd_json_variant **v) {
        ...
        sd_json_variant_propagate_sensitive(*v, n);
...
int sd_json_variant_normalize(sd_json_variant **v) {
        ...
        for (i = 0; i < m; i++) {
                ...
                r = sd_json_variant_normalize(a + i);
                if (r < 0)
                        goto finish;
        }
...
int sd_json_variant_unbase64(sd_json_variant *v, void **ret, size_t *ret_size) {
        ...
        return unbase64mem(sd_json_variant_string(v), SIZE_MAX, ret, ret_size);
}
...
int sd_json_variant_unhex(sd_json_variant *v, void **ret, size_t *ret_size) {
        ...
        return unhexmem(sd_json_variant_string(v), SIZE_MAX, ret, ret_size);
}
