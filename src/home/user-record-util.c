...
int user_record_synthesize(
        ...
        r = sd_json_build(&h->json,
                       SD_JSON_BUILD_OBJECT(
                                       ...
                                       SD_JSON_BUILD_PAIR("disposition", SD_JSON_BUILD_CONST_STRING("regular")),
        ...
        r = sd_json_build(&g->json,
                       SD_JSON_BUILD_OBJECT(
                                       ...
                                       SD_JSON_BUILD_PAIR("status", SD_JSON_BUILD_OBJECT(
                                                                       SD_JSON_BUILD_PAIR(SD_ID128_TO_STRING(mid), SD_JSON_BUILD_OBJECT(
                                                                                                       SD_JSON_BUILD_PAIR("service", SD_JSON_BUILD_CONST_STRING("io.systemd.Home"))))))));
        if (r < 0)
                return r;
...
int user_record_add_binding(
        ...
        r = sd_json_build(&new_binding_entry,
                       SD_JSON_BUILD_OBJECT(
                                       ...
                                       SD_JSON_BUILD_PAIR_CONDITION(!sd_id128_is_null(partition_uuid), "partitionUuid", SD_JSON_BUILD_STRING(ID128_TO_UUID_STRING(partition_uuid))),
                                       SD_JSON_BUILD_PAIR_CONDITION(!sd_id128_is_null(luks_uuid), "luksUuid", SD_JSON_BUILD_STRING(ID128_TO_UUID_STRING(luks_uuid))),
                                       SD_JSON_BUILD_PAIR_CONDITION(!sd_id128_is_null(fs_uuid), "fileSystemUuid", SD_JSON_BUILD_STRING(ID128_TO_UUID_STRING(fs_uuid))),
        ...
        if (binding) {
                ...
                if (be) {
                        r = sd_json_variant_merge(&be, new_binding_entry);
                        if (r < 0)
                                return r;
...
static bool per_machine_entry_empty(sd_json_variant *v) {
        ...
        SD_JSON_VARIANT_OBJECT_FOREACH(k, e, v)
...
int user_record_set_password_change_now(UserRecord *h, int b) {
        ...
        if (per_machine) {
                ...
                SD_JSON_VARIANT_ARRAY_FOREACH(e, per_machine) {
