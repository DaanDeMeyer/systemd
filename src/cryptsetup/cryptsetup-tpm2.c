...
int find_tpm2_auto_data(
        ...
        for (token = start_token; token < sym_crypt_token_max(CRYPT_LUKS2); token++) {
                ...
                if (w) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "TPM2 PCR bank invalid or not supported: %s", sd_json_variant_string(w));
                ...
                if (w) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "TPM2 primary key algorithm invalid or not supported: %s", sd_json_variant_string(w));
                ...
                r = unbase64mem(sd_json_variant_string(w), SIZE_MAX, &blob, &blob_size);
                if (r < 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                ...
                r = unhexmem(sd_json_variant_string(w), SIZE_MAX, &policy_hash, &policy_hash_size);
                if (r < 0)
                        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
